...
class PolicyEstimator():
    ...
    def __init__(self, num_outputs, reuse=False, trainable=True):
        ...
        with tf.variable_scope("policy_net"):
            ...
            self.entropy = - \
                tf.reduce_sum(self.probs * tf.log(self.probs),
                              1, name="entropy")
            ...
            gather_indices = tf.range(batch_size) * \
                tf.shape(self.probs)[1] + self.actions
            self.picked_action_probs = tf.gather(
                tf.reshape(self.probs, [-1]), gather_indices)

            self.losses = - (tf.log(self.picked_action_probs)
                             * self.targets + 0.01 * self.entropy)
            ...
            if trainable:
                ...
                self.grads_and_vars = self.optimizer.compute_gradients(
                    self.loss)
                self.grads_and_vars = [
                    [grad, var] for grad, var in self.grads_and_vars if grad is not None]
                self.train_op = self.optimizer.apply_gradients(
                    self.grads_and_vars,
                    global_step=tf.contrib.framework.get_global_step()
                )

        # Merge summaries from this network and the shared network (but not the
        # value net)
        var_scope_name = tf.get_variable_scope().name
        summary_ops = tf.get_collection(tf.GraphKeys.SUMMARIES)
        sumaries = [
            s for s in summary_ops if "policy_net" in s.name or "shared" in s.name]
    ...
    def __init__(self, reuse=False, trainable=True):
        ...
        with tf.variable_scope("value_net"):
            ...
            self.logits = tf.squeeze(
                self.logits, squeeze_dims=[1], name="logits")
            ...
            tf.summary.scalar("{}/max_value".format(prefix),
                              tf.reduce_max(self.logits))
            tf.summary.scalar("{}/min_value".format(prefix),
                              tf.reduce_min(self.logits))
            tf.summary.scalar("{}/mean_value".format(prefix),
                              tf.reduce_mean(self.logits))
            ...
            tf.summary.histogram(
                "{}/reward_targets".format(prefix), self.targets)
